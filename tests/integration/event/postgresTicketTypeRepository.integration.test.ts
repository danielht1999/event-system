// src/modules/event/infrastructure/repositories/PostgresTicketTypeRepository.integration.test.ts
...
describe('PostgresTicketTypeRepository (Integration)', () => {
  ...
  describe('findByEventId()', () => {
    it('debería recuperar todos los tickets del evento', async () => {
      ...
      expect(
        result.some(t => t.nombre === 'General')
      ).toBe(true);

      expect(
        result.some(t => t.nombre === 'VIP')
      ).toBe(true);
    });
  ...
  describe('findByIdForUpdate()', () => {
    it('debería recuperar ticket para bloqueo', async () => {
      const ticket = new TicketType(
        ticketId1,
        eventId,
        'VIP',
      ...
      const found = await repository.findByIdForUpdate(ticketId1);
    ...
    it('debería retornar null si no existe', async () => {
      const found =
        await repository.findByIdForUpdate(uuidv4());
